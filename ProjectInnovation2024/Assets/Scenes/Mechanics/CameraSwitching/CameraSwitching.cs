...
public class CameraSwitching : MonoBehaviour
{
    ...
    private void CheckCurrentScreen()
    {
        if (currentScreen < 0)
        {
            currentScreen = /*screens.Count*/ 4 - 1;
        }
    ...
    {
        int nextScreen = (currentScreen - 1 + /*screens.Count*/ 4) % /*screens.Count*/ 4;
        StartCoroutine(FadeToBlackAndSwitch(nextScreen));
    }

    public void RightArrowClick()
    {
        int nextScreen = (currentScreen + 1) % /*screens.Count*/ 4;
        StartCoroutine(FadeToBlackAndSwitch(nextScreen));
    }
/*
    public void OpenZoomPotion()
    {
        int zoomPotionScreen = 4;
        StartCoroutine(FadeToBlackAndSwitch(zoomPotionScreen));
    }

    public void OpenZoomDoor()
    {
        int zoomPotionScreen = 5;
        StartCoroutine(FadeToBlackAndSwitch(zoomPotionScreen));
    }*/
